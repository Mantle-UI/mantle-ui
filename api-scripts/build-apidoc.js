        ...
        project.children.forEach((module) => {
            ...
            module_component_group &&
                module_component_group.children.forEach((component) => {
                    ...
                    const component_props_ref = component.extendedTypes && component.extendedTypes[0].typeArguments && component.extendedTypes[0].typeArguments[0];
                    const component_props_id = component_props_ref && component_props_ref.reflection && component_props_ref.reflection.id;
                    ...
                    if (component_properties) {
                        ...
                        if (component_properties.type && component_properties.type.types) {
                            component_properties.type.types.forEach((type) => {
                                const type_target_id = type.reflection && type.reflection.id;
                                const type_props = module_properties_group.children.find((c) => (type_target_id ? c.id === type_target_id : true));
            ...
            module_types_group &&
                module_types_group.children.forEach((event) => {
                    ...
                    let values = event.type ? event.type.toString() : '';
                    const declaration = event.type && event.type.declaration;
